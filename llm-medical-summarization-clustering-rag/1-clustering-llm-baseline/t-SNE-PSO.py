...
def pso(X, n_components, verbose, perplexity, num_particles, max_iter, h, f, w, compute_error=True):
    ...
    for i in range(num_particles):
        ...
        particle = umap.UMAP(n_neighbors=perplexity, n_components=n_components, min_dist=0.0, metric='euclidean').fit_transform(X)
        ...
        for p in particles:
            ...
            if fitness < p['pbest_value'][i]:  # and is_feasible(Y[i, :]):
                ...
                if fitness < swarm_best_fitness:
                    ...
                    stepsize = np.sqrt(np.sum((p['gbest'] - p['Y']) ** 2))
...
embedding = pso(X_scaled, 2, compute_error=True ,verbose=1, perplexity=15, num_particles=5, max_iter=200, h=1e-20, f=1e-21, w=1e-20)
end_time = time.time() - time_start

df = pd.DataFrame(embedding)
df.to_csv("t-SNE-PSO-embedding-NC2.csv", index=False)

kmeans = cluster.KMeans(n_clusters=3)
y_pred = kmeans.fit_predict(embedding)

patient_dataset2["t-SNE-PSO_kmeans_labels"]=y_pred
patient_dataset2.to_csv("patient_dataset_with_reports.csv", index=False)

print("TSNE-PSO results")
...
#plt.title("t-SNE-PSO Visualization")
plt.xlabel("t-SNE-PSO First Component", fontsize=12)
plt.ylabel("t-SNE-PSO Second Component", fontsize=12)
plt.show()